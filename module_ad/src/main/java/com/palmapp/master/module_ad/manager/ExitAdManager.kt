 ...
 * @since :      2019/8/12
 */
private val DEFAULT_CONFIG = AdBean(MODEL_EXIT_ID, "1", 999, 0)

object ExitAdManager : AbsAdManager() {

    private val TAG = ExitAdManager.javaClass.name
    ...
    override fun isLoad(baseModuleDataItemBean: BaseModuleDataItemBean): Boolean {
        ...
        val lastAdShowTime = GoPrefManager.getDefault()
            .getLong(PreConstants.AD.KEY_EXIT_PAY_RESULT_LAST_SHOW_TIME, 0)//上次展示时间
        var adShowTimes =
            GoPrefManager.getDefault().getLong(PreConstants.AD.KEY_EXIT_PAY_RESULT_SHOW_TIMES, 0)//今天展示次数
        //step0判断是否打开开关
        if (!isOpenAd) {
            LogUtil.i(TAG, "ExitAdManager 不满足开关")
            return false
        }
        //step1判断上次展示时机是否为今天
        ...
        if (currentDay != lastDay) {
            adShowTimes = 0
            GoPrefManager.getDefault().putLong(PreConstants.AD.KEY_EXIT_PAY_RESULT_SHOW_TIMES, 0).apply()
        }
        //step2判断每天展示次数是否超过上限
        if (adShowTimes >= adShowCount) {
            LogUtil.i(TAG, "ExitAdManager 不满足展示次数")
            return false
        }
        //step3判断间隔时间
        if (currentTime - lastAdShowTime < adSplitTIme* 60 * 1000) {
            LogUtil.i(TAG, "ExitAdManager 不满足间隔时间")
            return false
        }
        return true
    }
    ...
    override fun onAdShow(adObj: Any?) {
        GoPrefManager.getDefault()
            .putLong(PreConstants.AD.KEY_EXIT_PAY_RESULT_LAST_SHOW_TIME, System.currentTimeMillis())
            .apply()
        var currentAdShowCount =
            GoPrefManager.getDefault().getInt(PreConstants.AD.KEY_EXIT_PAY_RESULT_SHOW_TIMES, 0)
        currentAdShowCount++
        GoPrefManager.getDefault()
            .putInt(PreConstants.AD.KEY_EXIT_PAY_RESULT_SHOW_TIMES, currentAdShowCount)
            .apply()
    }
    ...
    fun loadAd(activity: Activity) {
        ...
        mLoading = true
        super.loadAd(MODEL_EXIT_ID, activity)
    }
    ...
    fun getConfig(): AdBean {
        var adBean: AdBean =
            ConfigManager.getConfig(AdConfig::class.java)?.hashMap?.get(MODEL_EXIT_ID)
