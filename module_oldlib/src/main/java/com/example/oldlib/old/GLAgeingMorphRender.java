...
public class GLAgeingMorphRender {
...
//        GLES20.glEnableVertexAttribArray(mTextureCoordHandle2);
    ...
    public void draw(int srcTexture, float[] landMarks, int width, int height,
                     ...
                     float[] srcPoints,  float[] disPoints ,int[] targetPointIndexes) {

        int areaCount = 30; // 将图像切割成 5 * 5 的区域

        float[] sPoints = new float[srcPoints.length];
        float[] dPoints = new float[srcPoints.length];
        ...
        for (int k = 0; k < targetPointIndexes.length; k++) {
            ...
            float[] demosrcPoints = {srcPoints[k*2] , srcPoints[k*2+1]};
            float[] demodisPoints = {disPoints[k*2] , disPoints[k*2+1]};
            sPoints[k * 2] = pointData[0];
            sPoints[k * 2 + 1] = pointData[1];
            dPoints[k * 2] = demodisPoints[0] - demosrcPoints[0] + pointData[0];
            dPoints[k * 2 +1] =demodisPoints[1] - demosrcPoints[1] + pointData[1];
        }

        float[] gridPoints = createMeshPoints(textureWidth, textureHeight, areaCount);
        float[] result = MlsUtils.mlsWithRigid(maskTextureId, gridPoints, sPoints, dPoints, false);
