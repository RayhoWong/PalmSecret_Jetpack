...
public class PixelBuffer {
    ...
    public Bitmap getBitmapWithBoundary(@NonNull Rect boundary) {
        // Do we have a renderer?
        if (mRenderer == null) {
            Log.e(TAG, "getBitmap: Renderer was not set.");
            return null;
    ...
    public void destroy(boolean drawFrame) {
        if (drawFrame) {
            mRenderer.onDrawFrame(mGL);
            mRenderer.onDrawFrame(mGL);
        }
    ...
    private void convertToBitmap(@NonNull Rect boundary) {
        if (boundary.isEmpty()) {
            int[] iat = new int[mWidth * mHeight];
            IntBuffer ib = IntBuffer.allocate(mWidth * mHeight);
            mGL.glReadPixels(0, 0, mWidth, mHeight, GL_RGBA, GL_UNSIGNED_BYTE, ib);
            ...
            IntBuffer ib = IntBuffer.allocate(boundary.width() * boundary.height());
            mGL.glReadPixels(left, top, boundary.width(), boundary.height(), GL_RGBA, GL_UNSIGNED_BYTE, ib);
            mBitmap = Bitmap.createBitmap(boundary.width(), boundary.height(), Bitmap.Config.ARGB_8888);
