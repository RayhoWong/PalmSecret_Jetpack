...
import com.example.oldlib.old.TextureRotationUtil;
...
import static com.example.oldlib.old.GPUImageRenderer.CUBE;
import static com.example.oldlib.old.GPUImageRenderer.TEXTURE_NO_ROTATION;
        ...
        if (mMergedFilters != null) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                if (isNotLast) {
                    ...
                    if (i == 0) {
                        if (filter instanceof IDrawToFrameBufferFilter) {
                            ((IDrawToFrameBufferFilter)filter).setFinalOutputFrameBuffer(0, true); // 最后一个的渲染目的地是屏幕
                        }
                        filter.onDraw(previousTexture, cubeBuffer, textureBuffer);
                    } else {
                        if (filter instanceof IDrawToFrameBufferFilter) {
                            if (isNotLast) {
                                ((IDrawToFrameBufferFilter)filter).setFinalOutputFrameBuffer(mFrameBuffers[i], true); // 设置纹理滤镜的渲染目的地
                            } else {
                                ((IDrawToFrameBufferFilter)filter).setFinalOutputFrameBuffer(0, true); // 最后一个的渲染目的地是屏幕
                            }
                        }
