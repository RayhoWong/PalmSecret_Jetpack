...
//            postEvent(new CameraErrorEvent(ERROR_CODE_STATE_EXCEPTION));
            return false;
        }
        startOpeningCamera();
        return true;
    ...
    public boolean takePicture() {
        if (isCameraOpened()) {  //因为camera的open是异步回调，如果用户快速点击的话，就有可能发生崩溃
            if (mAutoFocus && getFacing() == FACING_BACK) {
                lockFocus();
            } else {
                captureStillPicture();
            }
...
//        }
        ...
        if (!mPreviewSizes.ratios().contains(mAspectRatio)) {//如果预览的尺寸不含默认的16:9
            if (mPreviewSizes.ratios().iterator().hasNext()) {
