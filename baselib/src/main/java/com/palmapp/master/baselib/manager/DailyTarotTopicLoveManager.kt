 ...
 * 真爱塔罗牌话题管理
 */
object DailyTarotTopicLoveManager {

    var cacheTarot = arrayListOf<TopicAngleAnswer>()  //缓存的塔罗牌
    var defaultTarot = arrayListOf<TopicAngleAnswer>() //默认第一次请求 获取的5张话题塔罗牌
    var result = "" //结果语
    ...
    fun loadData(listener: OnShowTarotTopicListener) {
        if (cacheTarot.isEmpty()) {
            ...
            val temp = GoGson.fromJson<ArrayList<TopicAngleAnswer>>(
                pref.getString(PreConstants.Today.KEY_TODAY_CACHE_TAROT_TOPIC, ""),
        ...
        if (defaultTarot.isEmpty()) {
            //获取第一次获取的塔罗牌
            val temp = GoGson.fromJson<ArrayList<TopicAngleAnswer>>(
                pref.getString(PreConstants.Today.KEY_TODAY_CACHE_TAROT_TOPIC_DEFAULT, ""),
                object : TypeToken<ArrayList<TopicAngleAnswer>>() {}.type
            )
            if (temp != null) {
                defaultTarot = temp
        ...
        //获取结果语
        result = pref.getString(PreConstants.Today.KEY_TODAY_CACHE_TAROT_TOPIC_RESULT, "")

        val cacheDay = pref.getInt(PreConstants.Today.KEY_TODAY_CACHE_TOPIC_LOVE_DAY_OF_YEAR, -1)
        ...
        if (currentDay != cacheDay) {
            ...
            defaultTarot.clear()
            result = ""
            pref.remove(PreConstants.Today.KEY_TODAY_CACHE_TAROT_TOPIC_RESULT)
            pref.remove(PreConstants.Today.KEY_TODAY_CACHE_TAROT_TOPIC)
            pref.remove(PreConstants.Today.KEY_TODAY_CACHE_TAROT_TOPIC_DEFAULT)
        }
        pref.putInt(PreConstants.Today.KEY_TODAY_CACHE_TOPIC_LOVE_DAY_OF_YEAR, currentDay).apply()
    ...
    fun storeDefaultTarotTopic(list: List<TopicAngleAnswer>, result: String) {
        if (list.isNotEmpty()) {
            ...
            pref.putString(
                PreConstants.Today.KEY_TODAY_CACHE_TAROT_TOPIC_DEFAULT,
                GoGson.toJson(defaultTarot)
            )
            pref.putString(
                PreConstants.Today.KEY_TODAY_CACHE_TAROT_TOPIC_RESULT,
                result
            )
            pref.apply()
        }
    ...
    fun storeCacheTarotTopic(list: List<TopicAngleAnswer>) {
        if (list.isNotEmpty()) {
            ...
            pref.putString(
                PreConstants.Today.KEY_TODAY_CACHE_TAROT_TOPIC,
