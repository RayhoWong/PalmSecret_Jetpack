...
//        try {
//            palmRecog = InterpreterHelper.decryptModel(GoCommonEnv.getApplication(), "palm_recog_5lines_192_input_1_output_0_encrypt")
...
//                love.addPoint(resultValues[i][j][index + 3], j, i)
//                business.addPoint(resultValues[i][j][index + 4], j, i)
//                money.addPoint(resultValues[i][j][index + 2], j, i)
//            }
//        }
//        if (life.isEmpty() || love.isEmpty() || business.isEmpty()) {
//            BaseSeq101OperationStatistic.uploadOperationStatisticData(BaseSeq101OperationStatistic.palm_succ, "1", "2", "2")
//        } else {
//            BaseSeq101OperationStatistic.uploadOperationStatisticData(BaseSeq101OperationStatistic.palm_succ, "1", "2", "1")
//        }
...
//        val temp = Bitmap.createBitmap(scaleBitmap)
//        val canvas = Canvas(temp)
//        val paint = Paint()
//        paint.textAlign = Paint.Align.CENTER
//        paint.textSize = 12f
//        for (i in 0 until keyPoints.size) {
//            canvas.drawText(i.toString(), keyPoints.get(i).x, keyPoints.get(i).y, paint)
//        }
//
//        FileUtils.writeBitmap(
//            GoCommonEnv.getApplication().cacheDir.path.plus(File.separator).plus("test.png"),
//            temp
//        )
//
...
//
//            when {
//                moneyUp.isNotEmpty() -> result.money_pos = KEY_MONEY_HIGH
//                moneyEq.isNotEmpty() -> result.money_pos = KEY_MONEY_EQULE
...
//            xB1 > xA1 && xB1 > xA1 -> result.palm_type = KEY_PALM_BIG
...
//
//        //判断食指长度、手掌宽度，手掌长度
//        result.finger_length = PalmCalcUtils.getDistance(keyPoints[8], keyPoints[11]).roundToInt()
//        result.palm_width = PalmCalcUtils.getDistance(keyPoints[3], keyPoints[20]).roundToInt()
//        result.palm_length = PalmCalcUtils.getDistance(keyPoints[20], keyPoints[21]).roundToInt()
//
...
//        if (value >= 0.25f) {
//            this.add(PointF(x.toFloat()*12, y.toFloat()*12))
//        }
//    }
//
...
////            )·
//            val temp = Bitmap.createBitmap(INPUT_SIZE * 12, INPUT_SIZE * 12, Bitmap.Config.ARGB_8888)
...
//                c.drawText(j.toString(), p.get(j).x, p.get(j).y, paint)
//            }
//            FileUtils.writeBitmap(
//                GoCommonEnv.getApplication().cacheDir.path.plus(File.separator).plus("${i}.png"),
//                temp
//            )
