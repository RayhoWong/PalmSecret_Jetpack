...
import com.palmapp.master.baselib.manager.DailyTarotTopicLoveManager
...
import kotlinx.android.synthetic.main.tarot_activity_topic_love.*
 ...
 * 塔罗话题 寻找真爱
 */
@Route(path = RouterConstants.ACTIVITY_TAROT_TOPIC_LOVE)
class TopicLoveActivity : BaseActivity(), TarotAdapter.OnItemClickListener,
    CommonDialog.OnClickBottomListener {
    ...
    private var card_5: TopicAngleAnswer? = null
    ...
    private var topic_name = "" //话题名称
    private var selectedCount = 0 //选中卡牌数量最大为5
    ...
    private var isShowedCard_5 = false
    private var clickType: Int = 0 //当前点击卡牌的类型


    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.tarot_activity_topic_love)
    ...
    private fun initView() {
        ...
        tv_titlebar_title.text = getString(R.string.tarot_love_title)
    ...
    private fun initCardClick() {
        ...
        iv_tarot_bottom.setOnClickListener {
            if (!card_5!!.isFlip) {
                return@setOnClickListener
            }
            showCardDialog(card_5!!)
        }
...
//        }
        ...
        when (selectedCount) {
            1 -> {
                ...
                startAnimation(itemview, position, card_1!!)
            }
            ...
            2 -> {
                ...
                startAnimation(itemview, position, card_2!!)
            }
            ...
            3 -> {
                ...
                startAnimation(itemview, position, card_3!!)
            }
            ...
            4 -> {
                ...
                startAnimation(itemview, position, card_4!!)
            }
            ...
            5 -> {
                ...
                isShowedCard_5 = true
                card_5?.isFlip = true
                mCacheTarotList.add(card_5!!)
                mList.forEach {
                    if (card_5?.card_key == it.card_key) {
                        flipImageView.setFlippedDrawable(resources.getDrawable(it.cover))
                        flipImageView.onClick(flipImageView)
                        tv_card_name.text = it.name
                        tv_keywords.text = it.keyword
                ...
                tv_topic.text = card_5?.topic
                tv_card_content.text = card_5?.explanation
                startAnimation(itemview, position, card_5!!)
            }
        }
    ...
    private fun getCacheTarot() {
        DailyTarotTopicLoveManager.loadData(object :
            DailyTarotTopicLoveManager.OnShowTarotTopicListener {
            override fun showTarot(
                default: List<TopicAngleAnswer>,
                list: List<TopicAngleAnswer>,
                result: String
            ) {
                if (list.isNotEmpty()) {
                    list.forEach {
                        when (it.angle_number) {
                            ...
                            5 -> {
                                card_5 = it
                                mList.forEach { bean ->
                                    if (card_5?.card_key == bean.card_key) {
                                        if (card_5?.card_place_type == AppConstants.TAROT_CARD_PLACE_TYPE_UPRIGHT) {
                                            iv_tarot_bottom.setImageResource(bean.cover)
                                        } else if (card_5?.card_place_type == AppConstants.TAROT_CARD_PLACE_TYPE_REVERSED) {
                                            iv_tarot_bottom.setImageResource(bean.cover)
                                            iv_tarot_bottom.rotation = 180f
                                        }
                                    }
                                }
                                selectedCount = 5
                ...
                if (mCacheTarotDefault.isNotEmpty()) {
                    for (item in mCacheTarotDefault) {
                        when (item.angle_number) {
                            ...
                            5 -> {
                                if (card_5 == null) card_5 = item
                            }
                        }
                    }

                    rl_tarot.visibility = View.VISIBLE
                    if (mCacheTarotList.isNotEmpty() && mCacheTarotList.size == 5) {
                        //用户已经选了5张 直接显示结果
                        showResult()
                    } else {
                        //否则显示卡牌列表布局
    ...
    private fun getTopicId() {
        ...
        request.tarot_card_array_id = arrayListOf(TarotTopicInfoRequest.tarot_card_array_id_love)
                    ...
                    override fun onNext(t: TarotTopicInfoResponse) {
                        if (t.status_result!!.isSuccess() && t.topic_info_list.isNotEmpty()) {
                            t.topic_info_list.forEach {
                                //只要真爱话题的id
                                if (it.topic_id == AppConstants.TAROT_TOPIC_LOVE) {
                ...
                .subscribe(object : NetworkSubscriber<TarotTopicInfoDetailResponse>() {
                    ...
                    override fun onNext(t: TarotTopicInfoDetailResponse) {
                        if (t.status_result!!.isSuccess() && t.topic_answer!!.topic_angle_answer_list!!.isNotEmpty()) {
                            ...
                            if (t.topic_angle_content_dto_list!!.isNotEmpty()) {
                                for (item in t.topic_angle_content_dto_list!!) {
                                    when (item.angle_number) {
                                        ...
                                        5 -> {
                                            if (card_5 != null) {
                                                card_5?.topic = item.content
                                                mCacheTarotDefault.add(card_5!!)
                                            }
                                        }
                                    }
                                }
                            ...
                            DailyTarotTopicLoveManager.storeDefaultTarotTopic(
                                mCacheTarotDefault,
                                mResult
                            )
                            tv_tarot_result.text = mResult
     ...
     * 开始动画
     */
    private fun startAnimation(itemView: View, position: Int, card: TopicAngleAnswer) {
        //禁止滑动和点击
...
//         x轴偏移量=卡牌中心点x坐标 - 屏幕中心点的x坐标
        ...
        when {
            dx > 0 -> {//大于0表示该张牌在屏幕中心点的右边
                translationX =
                    ObjectAnimator.ofFloat(itemView, "translationX", 0f, -dx.absoluteValue)
            }
        ...
        var translationY_1 =
            ObjectAnimator.ofFloat(
                ...
                "translationY",
                0f,
                -resources.getDimension(R.dimen.change_320px)
            )
        //中心位置偏移
        var translationY_2 =
            ObjectAnimator.ofFloat(
        ...
        translationX.addListener(object : Animator.AnimatorListener {
            ...
            override fun onAnimationEnd(animation: Animator?) {
                showCardDetail(itemView, position, card.angle_number)
            }
    ...
    private fun recoverAnimation(card: TopicAngleAnswer) {
        ...
        when (card.angle_number) {
            1 -> { //位置1
                ...
                var translationX =
                    ObjectAnimator.ofFloat(iv_replace1, "translationX", 0f, -dx.absoluteValue)
                //偏移量50 通过目测调试得出..
                var translationY = ObjectAnimator.ofFloat(iv_replace1, "translationY", 0f, 50f)
            ...
            2 -> { //位置2
                ...
                var dx = targetX - rl_tarot.left - resources.getDimension(R.dimen.change_105px)
                var translationX =
                    ObjectAnimator.ofFloat(iv_replace2, "translationX", 0f, dx.absoluteValue)
                //偏移量40 通过目测调试得出..
                var translationY = ObjectAnimator.ofFloat(iv_replace2, "translationY", 0f, 40f)
            ...
            3 -> { //位置3
                ...
                //偏移量450 通过目测调试得出..
                var translationY = ObjectAnimator.ofFloat(iv_replace3, "translationY", 0f, -450f)
            ...
            5 -> { //位置5
                isShowedCard_5 = false
                //偏移量450 通过目测调试得出..
                var translationY = ObjectAnimator.ofFloat(iv_replace5, "translationY", 0f, 450f)
                var scaleX = ObjectAnimator.ofFloat(iv_replace5, "scaleX", 1f, 0.394f)
                var scaleY = ObjectAnimator.ofFloat(iv_replace5, "scaleY", 1f, 0.394f)
                var alpha = ObjectAnimator.ofFloat(rl_cover, "alpha", 0.999f, 0.5f)
                animatorSet
                    .play(translationY)
                    .with(alpha)
                ...
                when (card.angle_number) {
                    ...
                    5 -> {
                        mList.forEach {
                            if (card_5?.card_key == it.card_key) {
                                if (card_5?.card_place_type == AppConstants.TAROT_CARD_PLACE_TYPE_UPRIGHT) {
                                    iv_tarot_bottom.setImageResource(it.cover)
                                } else if (card_5?.card_place_type == AppConstants.TAROT_CARD_PLACE_TYPE_REVERSED) {
                                    iv_tarot_bottom.setImageResource(it.cover)
                                    iv_tarot_bottom.rotation = 180f
                                }
    ...
    private fun closeReplace(angle_number: Int) {
        ...
        when (angle_number) {
            ...
            5 -> { //位置5
                iv_replace4.visibility = View.GONE
            }
        }
     ...
     * 显示卡牌详情
     */
    private fun showCardDetail(itemView: View, position: Int, angle_number: Int) {
        ...
        when (angle_number) {
            ...
            5 -> {
                mList.forEach {
                    if (card_5?.card_key == it.card_key) {
                        iv_replace5.visibility = View.VISIBLE
                        if (card_5?.card_place_type == AppConstants.TAROT_CARD_PLACE_TYPE_UPRIGHT) {
                            //正位
                            iv_replace5.setImageResource(it.cover)
                        } else if (card_5?.card_place_type == AppConstants.TAROT_CARD_PLACE_TYPE_REVERSED) {
                            //逆位
                            iv_replace5.setImageResource(it.cover)
                            iv_replace5.rotation = 180f
                        }
                    }
                }
            }
...
//            && card_3 != null && card_4 != null && card_5 != null
//        ) {
//
...
//            ShareManager.share(result, getString(R.string.app_app_name), 5)
//        }
//    }
...
//        iv_titlebar_share.visibility = View.VISIBLE
     ...
     */
    private fun exitTip() {
        //选中卡牌少于5张的时候 弹出dialog
        if (selectedCount in 1 until 5) {
            ...
            //选完5张 保存卡牌
            DailyTarotTopicLoveManager.storeCacheTarotTopic(mCacheTarotList)
            finish()
        }
    }
